...
# -----------------------
parties = {
    "CPN_UML": r"\bcpn\s?-?\s?uml\b|\buml\b",
    "Nepali_Congress": r"\bnepali\s?congress\b|\bnc\b",
    "RSP": r"\brastriya\s?swatantra\s?party\b|\brsp\b",
}
...
for party, pattern in parties.items():
    ...
    mention_counts[party] = len(sentiments)
...
for party, pattern in parties.items():
    ...
    S = (pos - neg) / total
    V = mention_counts[party] / all_mentions if all_mentions != 0 else 0
...
results_df.rename(columns={"index": "Party"}, inplace=True)
results_df.to_csv("party_mci_v2_results.csv", index=False)

print("\n=== PARTY MEDIA CONFIDENCE INDEX v2 ===")
